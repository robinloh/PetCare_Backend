...
queries.query = {
    ...
    find_services: 'SELECT A.email, u.name, bidamt.bidamount, avgrating, bidamt.dateofservice FROM Availabilities A NATURAL JOIN provideService S NATURAL JOIN Users u LEFT JOIN (SELECT R.email, AVG(rating) AS avgrating from reviews R GROUP BY R.email) as AR on AR.email = A.email left join (select bidamount, caretakeremail, dateofservice from Bids b where dateofservice = $1 and status = \'current highest\') as bidamt on bidamt.caretakeremail = A.email WHERE $1 BETWEEN startDate AND endDate AND case when $2::text = \'null\' then true else serviceid = $2 end AND avgrating >= $4 AND bidamt.bidamount < $3 AND NOT EXISTS (SELECT 1 FROM Bids B WHERE A.email = B.caretakerEmail AND dateOfService = $1 AND status = \'Won\')', // [dateOfService, typeOfService, bidamt, rating]
